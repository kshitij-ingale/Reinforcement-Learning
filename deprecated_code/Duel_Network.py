# This implementation is based on dueling network architecture as proposed by Wang et al(https://arxiv.org/abs/1511.06581) for OpenAI Gym environment
...
#        Stream to calculate value fuction
        value_layer = Dense(1)(hidden_layer3)
#        Stream to calculate advantage on account of that action
        advantage_layer = Dense(env.action_space.n)(hidden_layer3)
#        Normalize advantage layer by using mean
        norm_advantage = Lambda(lambda x: x - K.mean(x))(advantage_layer)
#        Finding Q value from value function and normalized advantage
        added = Add()([norm_advantage, value_layer])
#        Output the Q values
        output_layer = Dense(env.action_space.n)(added)
    ...
    def train(self):
        ...
        if self.render_decision:
            ...
            def f(x):
                return x%3333==0
            self.env.render()
            path ='video_duel_double_DQN_'+self.env_name+'/'
...
#                Save model after every 1000 episodes
               if episode%1000==0:
                   name = 'duel_double_DQN_model_'+self.env_name+'_'+str(episode)+'.h5'
...
def main(args):
    ...
    file_name = 'duel_double_DQN_plot_'+environment_name+'.png'
