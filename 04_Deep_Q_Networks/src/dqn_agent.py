...
from network import Net, train_step, update_target_model
...
class DQN:
    ...
    def train(self):
        ...
        lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
            float(NetworkParameters.QNet.learning_rate),
            decay_steps=1000,
            decay_rate=0.95,
            staircase=True,
        )
        ...
        anneal_eps = (
            epsilon - TrainingParameters.Exploration.final_epsilon
        ) / TrainingParameters.Exploration.decay_steps
        ...
        for episode in tqdm(
            ...
            while not done:
                ...
                next_states, rewards, states, actions, if_not_terminals = map(
                    np.array, zip(*batch)
                )
                if self.double:
                    """
                    ...
                    next_actions_indices = np.vstack(
                        [np.arange(TrainingParameters.batch_size), next_actions]
                    ).T
                    # Value is estimated using target QNet model instance
                    target_q_next_all_actions = self.target_Q_net(next_states)
                    targets = rewards + if_not_terminals * self.discount * tf.gather_nd(
            ...
            if episode % self.update_target_frequency == 0:
                update_target_model(
                    self.QNet,
                    self.target_Q_net,
                    smoothing=TrainingParameters.update_smoothing,
                )

        self.QNet.save_weights(
            os.path.join(Directories.output, "saved_models", f"model_{episode:05}")
        )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="DQN algorithm implementation: "
